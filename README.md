...
use Stephenjude\FilamentDebugger\DebuggerPlugin;
 ...
 $panel->plugin(
    DebuggerPlugin::make()
        ->authorize(condition:fn()=>filament()->auth()->user()->can('view.debuggers') )
);
 ```
...
$panel->plugin(
    DebuggerPlugin::make()
        ->horizonNavigation(
            condition: fn () => filament()->auth()->user()->can('view.horizon'),
            label: 'Horizon',
        ...
        ->telescopeNavigation(
            condition: fn()=> filament()->auth()->user()->can('view.telescope'),
            label: 'Telescope',
        ...
        ->pulseNavigation(
            condition: fn () => filament()->auth()->user()->can('view.pulse'),
            label: 'Pulse',
